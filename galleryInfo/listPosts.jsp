                    	...
                    	<c:choose>
                    		...
                    		<c:when test="${not empty galleryList}">
                    			<c:forEach var="post" items="${galleryList}" varStatus="postNum">
                    				<tr class="gallery_list">
                            			<td class="gallery_num">${((pageNum)-1)*6+postNum.count}</td>
                	...
                	<c:if test="${totPosts != 0}">
               			...
               			<c:forEach var="page" begin="1" end="${tot-(section-1)*6}" step="1">
               				<c:if test="${not doneLoop }">
	               				...
	               				<%-- 이전 섹션으로 돌아가는 버튼 --%>
	               					<a class="btn_prev" href="${contextPath}/gallery/listPosts.do?section=${section-1}&pageNum=${(section-1)*6}">&lt;</a>
	               				</c:if>
	               				<c:choose>
	          						<c:when test="${page==pageNum}">
	          							<%-- 현재 보고 있는 페이지 번호 --%>
	          							<a class="num click" href="${contextPath}/gallery/listPosts.do?section=${section}&pageNum=${page}">${(section-1)*6+page}</a>
	          						</c:when>
	          						<c:when test="${page!=pageNum}">
	          							<a class="num" href="${contextPath}/gallery/listPosts.do?section=${section}&pageNum=${page}">${(section-1)*6+page}</a>
	          						</c:when>
	               				</c:choose>
	               				<c:if test="${page == 5}">
	               					<%-- 다음 섹션으로 넘어가는 버튼 --%>
	               					<a class="btn_next" href="${contextPath}/gallery/listPosts.do?section=${section+1}&pageNum=${(section-1)*6+1}">&gt;</a>
               			...
               			<c:when test="${totPosts <= 30}">
	               			...
	               			<c:forEach var="page" begin="1" end="${tot}" step="1">	               				
	               				<c:choose>
	          						<c:when test="${page==pageNum}">
	          							<%-- 현재 보고 있는 페이지 번호 --%>
	          							<a class="num click" href="${contextPath}/gallery/listPosts.do?section=${section}&pageNum=${page}">${page}</a>
	          						</c:when>
	          						<c:otherwise>
	          							<a class="num" href="${contextPath}/gallery/listPosts.do?section=${section}&pageNum=${page}">${page}</a>
	          						</c:otherwise>
	               				</c:choose>	               				           		         
	               			</c:forEach>
               			</c:when>               			
