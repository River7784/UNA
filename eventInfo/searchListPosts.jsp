...
<c:set var="searchtype" value="${eventMap.searchtype}"/>
<c:set var="keyword" value="${eventMap.keyword}"/>
<%
                    		...
                    		<c:when test="${not empty eventList}">
                    			<c:forEach var="post" items="${eventList}" varStatus="postNum">
                    				<tr class="event_list">
                            			...
                            			<td class="event_num">${totPosts-(pageNum-1)*5+postNum.index}</td>
               		...
               		<c:choose>
               			...
               			<c:forEach var="page" begin="1" end="${tot-(section-1)*5}" step="1">
               				<c:if test="${not doneLoop }">
	               				...
	               				<%-- 이전 섹션으로 돌아가는 버튼 --%>
	               					<a class="btn_prev" href="${contextPath}/event/search.do?section=${section-1}&pageNum=${(section-1)*5}&searchtype=${searchtype}&keyword=${keyword}">&lt;</a>
	               				</c:if>
	               				<c:choose>
	          						<c:when test="${page==pageNum}">
	          							<%-- 현재 보고 있는 페이지 번호 --%>
	          							<a class="num click" href="${contextPath}/event/search.do?section=${section}&pageNum=${page}&searchtype=${searchtype}&keyword=${keyword}">${(section-1)*5+page}</a>
	          						</c:when>
	          						<c:when test="${page!=pageNum}">
	          							<a class="num" href="${contextPath}/event/search.do?section=${section}&pageNum=${page}&searchtype=${searchtype}&keyword=${keyword}">${(section-1)*5+page}</a>
	          						</c:when>
	               				</c:choose>
	               				<c:if test="${page == 5}">
	               					<%-- 다음 섹션으로 넘어가는 버튼 --%>
	               					<a class="btn_next" href="${contextPath}/event/search.do?section=${section+1}&pageNum=${(section-1)*5+1}&searchtype=${searchtype}&keyword=${keyword}">&gt;</a>
               			...
               			<c:when test="${totPosts <= 25}">
	               			...
	               			<c:forEach var="page" begin="1" end="${tot}" step="1">	               				
	               				<c:choose>
	          						<c:when test="${page==pageNum}">
	          							<%-- 현재 보고 있는 페이지 번호 --%>
	          							<a class="num click" href="${contextPath}/event/search.do?section=${section}&pageNum=${page}&searchtype=${searchtype}&keyword=${keyword}">${page}</a>
	          						</c:when>
	          						<c:otherwise>
	          							<a class="num" href="${contextPath}/event/search.do?section=${section}&pageNum=${page}&searchtype=${searchtype}&keyword=${keyword}">${page}</a>
	          						</c:otherwise>
	               				</c:choose>	               				           		         
	               			</c:forEach>
               			</c:when>               			
