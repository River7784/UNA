...
<c:set var="searchtype" value="${marketMap.searchtype}"/>
<c:set var="keyword" value="${marketMap.keyword}"/>
<%
                	...
                	<c:if test="${totPosts != 0 }">
               			...
               			<c:forEach var="page" begin="1" end="${tot-(section-1)*6}" step="1">
               				<c:if test="${not doneLoop }">
	               				...
	               				<%-- 이전 섹션으로 돌아가는 버튼 --%>
	               					<a class="btn_prev" href="${contextPath}/market/search.do?section=${section-1}&pageNum=${(section-1)*6}&searchtype=${searchtype}&keyword=${keyword}">&lt;</a>
	               				</c:if>
	               				<c:choose>
	          						<c:when test="${page==pageNum}">
	          							<%-- 현재 보고 있는 페이지 번호 --%>
	          							<a class="num click" href="${contextPath}/market/search.do?section=${section}&pageNum=${page}&searchtype=${searchtype}&keyword=${keyword}">${(section-1)*6+page}</a>
	          						</c:when>
	          						<c:when test="${page!=pageNum}">
	          							<a class="num" href="${contextPath}/market/search.do?section=${section}&pageNum=${page}&searchtype=${searchtype}&keyword=${keyword}">${(section-1)*6+page}</a>
	          						</c:when>
	               				</c:choose>
	               				<c:if test="${page == 6}">
	               					<%-- 다음 섹션으로 넘어가는 버튼 --%>
	               					<a class="btn_next" href="${contextPath}/market/search.do?section=${section+1}&pageNum=${(section-1)*6+1}&searchtype=${searchtype}&keyword=${keyword}">&gt;</a>
               			...
               			<c:when test="${totPosts <= 30}">
	               			...
	               			<c:forEach var="page" begin="1" end="${tot}" step="1">	               				
	               				<c:choose>
	          						<c:when test="${page==pageNum}">
	          							<%-- 현재 보고 있는 페이지 번호 --%>
	          							<a class="num click" href="${contextPath}/market/search.do?section=${section}&pageNum=${page}&searchtype=${searchtype}&keyword=${keyword}">${page}</a>
	          						</c:when>
	          						<c:otherwise>
	          							<a class="num" href="${contextPath}/market/search.do?section=${section}&pageNum=${page}&searchtype=${searchtype}&keyword=${keyword}">${page}</a>
	          						</c:otherwise>
	               				</c:choose>	               				           		         
	               			</c:forEach>
               			</c:when>               			
