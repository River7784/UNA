...
<c:set var="newsList" value="${newsMap.newsList}"/>
<c:set var="totPosts" value="${newsMap.totPosts}"/>
<c:set var="section" value="${newsMap.section}"/>
<c:set var="pageNum" value="${newsMap.pageNum}"/>
<%
...
<link rel="stylesheet" href="${contextPath}/resources/css/sub_news.css">
...
<script src="${contextPath}/resources/js/sub_news.js"></script>
...
<body>
    ...
    <div id="wrap">
        ...
        <div id="title_bg_wrap">
            <h2 id="sub_page_h2">신기해요 소식마당</h2>
        </div>
        <!---------- [S] #container ---------->
        <div id="news_wrap">
            <!-- 게시판 테이블 -->
            <div id="news_table_wrap">
                <!-- 게시판 테이블 리스트 영역 시작 -->
                <table class="news_table_list">
                    <caption>소식마당 게시판 목록</caption>
                    <thead>
                        <tr class="news_top hidden">
                            <th scope="col">번호</th>
                            <th scope="col">제목</th>
                            <th scope="col">조회수</th>
                            <th scope="col">작성일</th>
                    ...
                    <tbody>
                    	<c:choose>
                    		<c:when test="${empty newsList}">
                    			<tr class="news_list">
                    				<td class="news_list_empty" colspan="4">등록된 글이 없습니다.</td>
                    			</tr>
                    		</c:when>
                    		<c:when test="${not empty newsList}">
                    			<c:forEach var="post" items="${newsList}" varStatus="postNum">
                    				<tr class="news_list">
                            			<td class="news_num">${((pageNum)-1)*5+postNum.count}</td>
			                            <td class="news_title">
			                                <a href="${contextPath}/news/viewPost.do?num=${post.num}" class="news_img_link">
			                                    <img src="${contextPath}/ndownload.do?imageFileName=${post.imageFileName}&num=${post.num}" alt="기사사진">
			                                </a>
			                                <a href="${contextPath}/news/viewPost.do?num=${post.num}" class="news_title_link">${post.title}</a>
			                                <span class="news_title_prev">
												<c:choose>
													<c:when test="${fn:length(post.content) > 20}">
														<c:out value="${fn:substring(post.content, 0, 19)}"/>...
													</c:when>
													<c:otherwise>
                            			...
                            			<td class="news_date">${post.writeDate}</td>
                        			</tr>
                    			</c:forEach>
                    		</c:when>
                    	</c:choose>                                                
            ...
            <!-- 게시판 버튼 영역 시작 -->
            <div id="news_btn_wrap">
                <!-- 글쓰기 버튼 (관리자로 접속시 노출) -->  
                	<c:if test="${isLogon == true && log_id == 'admin'}">              
                	<div id="news_write">
                    	<a href="${contextPath}/news/writeForm.do" class="news_write_btn">글쓰기</a>
                	</div>      
                	</c:if>             
                <!-- 페이지 버튼 -->
                <div id="news_page">                	
               			...
               			<%-- 25개 초과 게시글이 있을 경우 --%>
	               			...
	               			<c:forEach var="page" begin="1" end="${tot-(section-1)*5}" step="1">
	               				<c:if test="${not doneLoop }">
		               				...
		               				<%-- 이전 섹션으로 돌아가는 버튼 --%>
		               					<a class="btn_prev" href="${contextPath}/news/listPosts.do?section=${section-1}&pageNum=${(section-1)*5}">&lt;</a>
		               				</c:if>
		               				<c:choose>
		          						<c:when test="${page==pageNum}">
		          							<%-- 현재 보고 있는 페이지 번호 --%>
		          							<a class="num click" href="${contextPath}/news/listPosts.do?section=${section}&pageNum=${page}">${(section-1)*5+page}</a>
		          						</c:when>
		          						<c:when test="${page!=pageNum}">
		          							<a class="num" href="${contextPath}/news/listPosts.do?section=${section}&pageNum=${page}">${(section-1)*5+page}</a>
		          						</c:when>
		               				</c:choose>
		               				<c:if test="${page == 5}">
		               					<%-- 다음 섹션으로 넘어가는 버튼 --%>
		               					<a class="btn_next" href="${contextPath}/news/listPosts.do?section=${section+1}&pageNum=${(section-1)*5+1}">&gt;</a>
               			...
               			<c:when test="${totPosts <= 25}">
	               			...
	               			<c:forEach var="page" begin="1" end="${tot}" step="1">	               				
	               				<c:choose>
	          						<c:when test="${page==pageNum}">
	          							<%-- 현재 보고 있는 페이지 번호 --%>
	          							<a class="num click" href="${contextPath}/news/listPosts.do?section=${section}&pageNum=${page}">${page}</a>
	          						</c:when>
	          						<c:otherwise>
	          							<a class="num" href="${contextPath}/news/listPosts.do?section=${section}&pageNum=${page}">${page}</a>
	          						</c:otherwise>
	               				</c:choose>	               				           		         
	               			</c:forEach>
               			</c:when>               			
                ...
                <!-- 검색 버튼 -->
                <div id="news_search_wrap">
                    <form action="${contextPath}/news/search.do" method="post">
                       <fieldset>
                           <legend>참여마당 검색 영역</legend>
                           <select name="searchtype" id="news_search_menu">
                               <option value="selectAll">전체</option>
                               <option value="selectByPost">글제목+내용</option>
                           </select>
                           <input type="search" name="keyword" id="news_search_content">
                           <input type="submit" id="news_search_btn" value="검색">
                       </fieldset>
                    </form>
                </div>
            </div>
